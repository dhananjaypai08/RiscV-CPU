...
impl Cpu {
    ...
    fn execute_op_imm(&mut self, inst: u32, rd: usize, funct3: u32, rs1: usize) -> Result<(), String> {
        ...
        let result = match funct3 {
            0b000 => rs1_val.wrapping_add(imm),
            0b010 => if (rs1_val as i64) < (imm as i64) { 1 } else { 0 },
            0b011 => if rs1_val < imm { 1 } else { 0 },
            0b100 => rs1_val ^ imm,
            0b110 => rs1_val | imm,
            0b111 => rs1_val & imm,
            0b001 => {
    ...
    fn execute_op(&mut self, rd: usize, funct3: u32, rs1: usize, rs2: usize, funct7: u32) -> Result<(), String> {
        ...
        let result = match (funct7, funct3) {
            (0b0000000, 0b000) => rs1_val.wrapping_add(rs2_val),
            (0b0100000, 0b000) => rs1_val.wrapping_sub(rs2_val),
            (0b0000000, 0b001) => rs1_val << (rs2_val & 0x3f),
            (0b0000000, 0b010) => if (rs1_val as i64) < (rs2_val as i64) { 1 } else { 0 },
            (0b0000000, 0b011) => if rs1_val < rs2_val { 1 } else { 0 },
            (0b0000000, 0b100) => rs1_val ^ rs2_val,
            (0b0000000, 0b101) => rs1_val >> (rs2_val & 0x3f),
            (0b0100000, 0b101) => ((rs1_val as i64) >> (rs2_val & 0x3f)) as u64,
            (0b0000000, 0b110) => rs1_val | rs2_val,
            (0b0000000, 0b111) => rs1_val & rs2_val,
...
pub mod extended_cpu;
fn main() {
    let mut cpu = Cpu::new();

    let program = vec![
        0x13, 0x01, 0xa0, 0x02,  // addi x1, x0, 42    (x1 = 42)
        0x93, 0x01, 0x50, 0x01,  // addi x2, x0, 21    (x2 = 21)
        0x33, 0x81, 0x20, 0x00,  // add x3, x1, x2     (x3 = x1 + x2 = 63)
        0xb3, 0x01, 0x21, 0x40,  // sub x4, x2, x1     (x4 = x2 - x1 = -21)
        0x33, 0x42, 0x21, 0x00,  // xor x5, x2, x1     (x5 = x2 ^ x1)
    ];
    ...
    println!("\nVerifying results:");
    println!("x1 should be 42: {}", cpu.read_register(1));
    println!("x2 should be 21: {}", cpu.read_register(2));
